...
def validate_all(expr_or_eq: sp.Basic | sp.Equality,
                 units: Dict[str, str],
    ...
    report = {"ok": True, "errors": [], "details": {}}

    # 1) Dimensional check (fast)
    if "dimensional" in checks:
        try:
            validate_dimensions(expr_or_eq, units, result_unit)
            report["details"]["dimensional"] = {"ok": True}
        except Exception as e:
            report["ok"] = False
            report["errors"].append({"stage": "dimensional", "message": str(e)})
    ...
    if "constraints" in checks:
        try:
            ...
            report["details"]["constraints"] = {"ok": True}
        except ConstraintError as ce:
            report["ok"] = False
            report["errors"].append({"stage": "constraints", "message": str(ce)})
            return report
    ...
    if domain == "relativistic" and "relativistic" in checks:
        try:
            ...
            report["details"]["relativistic"] = {"ok": True}
        except RelativisticError as re:
            report["ok"] = False
            report["errors"].append({"stage": "relativistic", "message": str(re)})
            return report
    ...
    if "pde_class" in checks:
        ...
        report["details"]["pde_class"] = {"ok": True, "class": pclass.value}
