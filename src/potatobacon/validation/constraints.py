...
def validate_constraints(expr: sp.Basic,
                         constraints: Dict[str, Any] | None = None,
                         require_real: bool = True) -> None:
    """
    Checks:
    ...
    if require_real and expr.is_real is False:
        violations.append(ConstraintViolation("realness", None,
                         "Expression may be complex (failed symbolic realness check)."))
    ...
    for sym in sorted(expr.free_symbols, key=lambda s: s.name):
        ...
        if symc.get("positive") and sym.is_positive is False:
            violations.append(ConstraintViolation("positive", str(sym),
                             f"{sym} must be positive (assumption violated)."))
        if symc.get("nonnegative") and sym.is_nonnegative is False:
            violations.append(ConstraintViolation("nonnegative", str(sym),
                             f"{sym} must be nonnegative (assumption violated)."))
        ...
        for i in range(min(5, max(len(g) for g in grids))):
            ...
            for s in free_syms:
                ...
                if sc.get("positive") and not (v > 0):
                    violations.append(ConstraintViolation("positive", str(s),
                                     f"{s} sampled value {v} is not positive."))
                    break
                if sc.get("nonnegative") and not (v >= 0):
                    violations.append(ConstraintViolation("nonnegative", str(s),
                                     f"{s} sampled value {v} is negative."))
                    break
